...
select pg_temp.must_fail(format($s$ select public.apply_changes(inserts => jsonb_build_array(jsonb_build_object('title','x','priority',1,'category_id','%s','due_date','2026-01-01','checklist',(select jsonb_agg(jsonb_build_object('text','i','done',false)) from generate_series(1,101))))) $s$, (select id from public.categories where name='B')), '101 checklist items');
select pg_temp.must_fail(format($s$ select public.apply_changes(inserts => jsonb_build_array(jsonb_build_object('title','x','priority',1,'category_id','%s','due_date','2026-01-01','notes',repeat('n',20001)))) $s$, (select id from public.categories where name='B')), 'notes too long');
